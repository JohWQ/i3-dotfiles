...
run_cmd() {
	...
	if [[ "$selected" == "$yes" ]]; then
		if [[ $1 == '--shutdown' ]]; then
			~/.config/scripts/i3status-rs_poweroff.sh
		elif [[ $1 == '--reboot' ]]; then
			~/.config/scripts/i3status-rs_reboot.sh
		elif [[ $1 == '--suspend' ]]; then
			~/.config/scripts/i3status-rs_suspend.sh
		elif [[ $1 == '--logout' ]]; then
			~/.config/scripts/i3status-rs_exit.sh
		fi
	else
		exit 0
	fi
...
case ${chosen} in
    ...
    $reboot)
        ...
        ;;
    $lock)
		~/.config/scripts/i3status-rs_lock.sh
        ;;
    $suspend)
		run_cmd --suspend
        ;;
